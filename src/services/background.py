...
from src.common.db import engine, Config, Device, Trace, JobRequest
...
if os.path.exists(LAST_MESSAGE_ID_FILE):
    try:
        ...
        LAST_MESSAGE_ID = '0'
        print(f"[BACKGROUND] Starting fresh with 0 (will read all messages)")
else:
    LAST_MESSAGE_ID = '0'
    print(f"[BACKGROUND] First run, using 0 to read all messages")
...
def process_job_request(job_id: str, config_id: str, device_serials: list):
    ...
    try:
        ...
        for device_serial in device_serials:
            try:
                print(f"Processing device {device_serial} for job {job_id}")

                # Send status update
                if redis_client:
                    send_job_update(redis_client, job_id, device_serial, "starting",
                                    f"Starting trace collection on {device_serial}")

                # Check if device exists in database
                device = session.exec(
                    select(Device).where(
                        (Device.device_uuid == device_serial) | (Device.device_id == device_serial)
                    )
                ).first()
                ...
                # Upload to MinIO
                if redis_client:
                    send_job_update(redis_client, job_id, device_serial, "uploading",
                ...
                print(f"Successfully created trace {trace.trace_id} for device {device_serial}")

                if redis_client:
                    send_job_update(redis_client, job_id, device_serial, "completed",
                                    f"Trace collected successfully", trace_id=trace.trace_id)
...
def send_job_update(redis_client, job_id: str, device_serial: str, status: str,
...
def background_task():
    ...
    try:
        ...
        try:
            ...
            if saved_position:
                current_position = saved_position.decode('utf-8') if isinstance(saved_position,
                                                                                bytes) else saved_position
            else:
                current_position = '0'
        except:
            current_position = '0'
        ...
        for stream_name, messages in streams:
            for message_id, fields in messages:
                try:
                    ...
                    lock_acquired = redis_client.client.set(lock_key, os.getpid(), nx=True, ex=60)
                    ...
                    print(f"[BACKGROUND] Worker {os.getpid()} acquired lock for message {message_id}")
                    ...
                    if not all([job_id, config_id, devices]):
                        print(f"[BACKGROUND] Invalid job request message: {decoded_fields}")
                    ...
                    def process_in_thread():
                        try:
                            ...
                            print(f"[BACKGROUND] Error in thread processing job {job_id}: {e}")
                            import traceback
                            traceback.print_exc()
                        finally:
                            ...
                            try:
                                redis_client.client.delete(lock_key)
                                print(f"[BACKGROUND] Worker {os.getpid()} released lock for job {job_id}")
                            except:
                                pass

                    thread = threading.Thread(target=process_in_thread, daemon=True)
                    thread.start()
                    print(f"[BACKGROUND] Worker {os.getpid()} started thread for job {job_id}")

                    # Update position in Redis IMMEDIATELY so other workers skip this message
                    redis_client.client.set('job_processor_position', message_id)
                    print(f"[BACKGROUND] Worker {os.getpid()} updated position to {message_id}")
