...
from src.common.db import JobRequest, Device
...
class JobRequestService:
    ...
    def create_job_request(self, config_id: str, device_serials: List[str]) -> JobRequest:
        ...
        job_request = JobRequest(
            job_id=job_id,
            config_id=config_id,
            device_serials=json.dumps(device_serials),
            status="pending"
        )
        ...
        if self.redis_client:
            ...
            try:
                self.redis_client.client.xadd(JOB_REQUEST_STREAM_NAME, stream_data)
    ...
    def get_job_request(self, job_id: str) -> Optional[JobRequest]:
        ...
        return self.session.exec(select(JobRequest).where(JobRequest.job_id == job_id)).first()

    def update_job_status(self, job_id: str, status: str, result_summary: Optional[str] = None):
    ...
    def get_job_updates_stream(self, job_id: str):
        ...
        while True:
            try:
                ...
                streams = self.redis_client.client.xread({stream_key: last_id}, block=1000)
    ...
    def send_job_update(self, job_id: str, device_serial: str, status: str, message: str = "",
    ...
    def get_all_devices_for_job(self, job_request: JobRequest) -> List[Device]:
        ...
        devices = self.session.exec(
            select(Device).where(Device.device_uuid.in_(device_serials) | Device.device_id.in_(device_serials))
        ).all()
        return list(devices)
